...
const updateUserProfile = async (req, res) => {
  const { bio, age, gender, location, interests, userId } = req.body;

  try {
    let interestsArray = Array.isArray(interests) ? interests : [interests];

    await db.query(
      "UPDATE userprofile SET bio = $1, age = $2, gender = $3, location = $4, interests = $5 WHERE userid = $6",
      [bio, age, gender, location, interestsArray, userId]
    );
