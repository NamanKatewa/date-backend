...
const notifications = async (req, res) => {
  ...
  try {
    const likedUsers = await db.query(
      "SELECT u.name FROM swipes s INNER JOIN users u ON s.sender_id = u.id WHERE s.receiver_id = $1 AND s.event = 'like'",
      [userId]
    );

    const likedUserNames = likedUsers.rows.map((row) => row.name);

    res.status(200).json({ likedUsers: likedUserNames });
  } catch (error) {
